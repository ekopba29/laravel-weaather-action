...
<template>
    ...
    <div v-if="loading">
        <div class="min-h-screen flex items-center justify-center">
            <span class="font-semibold text-lg animate-ping duration-300 animate-bounce">
                Loading...
            </span>
        </div>
    </div>

    <div class="min-h-screen flex items-center justify-center" v-else-if="!err">
        <div class="flex flex-col bg-white rounded p-4 w-full max-w-xs flex-wrap">

            <div class="font-bold text-xl">{{ city }}</div>
            <div class="text-sm text-gray-500">{{ localDate }}</div>

            <div class="flex flex-row items-center justify-center mt-2">
                <div class="font-medium text-6xl">
                    <img v-if="weather.icon" :src="cdnIconWeather(weather.icon)" class="w-20">
                ...
                <div class="flex flex-col items-end ml-6 ">
                    ...
                    <div class="mt-1 text-right">
                        <span class="text-sm font-light text-gray-500 capitalize">{{ weather.description }}</span>
                    </div>
                </div>
            </div>

            <div class="flex flex-row justify-between mt-6 flex-wrap">
                <div class="flex flex-col items-center">
                    <div class="font-medium text-sm">Wind</div>
                    <div class="text-sm text-gray-500 lining-nums">{{ wind.speed }} M/s</div>
                </div>
                <div class="flex flex-col items-center">
                    <div class="font-medium text-sm">Humidity</div>
                    <div class="text-sm text-gray-500 lining-nums">{{ main.humidity }}%</div>
                </div>
                <div class="flex flex-col items-center">
                    <div class="font-medium text-sm">Visibility</div>
                    <div class="text-sm text-gray-500 lining-nums">{{ visibilityInKM }}Km</div>
                </div>
            </div>
    ...
    <div v-else class="min-h-screen flex items-center justify-center flex-col">
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
            class="flex flex-col bg-white rounded p-4 w-full max-w-xs flex-wrap" viewBox="0 0 788.67 600.83">
            <path
            ...
            <polygon
                ...
                fill="#6c63ff" />
            <path d="m479.26,403.83c10.07,13.27,14.29,45.22,18.57,77,0,0-186-1-187-1s28.43-67,28.43-67l140-9h0Z"
                fill="#2f2e41" />
            <path
                d="m485.34,472.35l12.5,8.48s177-12,162,44c-15,56-62,65-62,65l-190,11-2-31s-183,41-200-43,105-47,105-47l174.5-7.48h0Z"
                fill="#2f2e41" />
            <path d="m410.65,538.83s4.19-22,34.19-13,47,21,47,21l-72,18-9.19-26Z" fill="#a0616a" />
            <path d="m485.69,231.49s34-12,47,27,10,71,10,71l-50,16-7-114h0Z" fill="#6c63ff" />
            <path
                d="m252.54,475.25l-10.91-19.22,23.57-58.67,11.08-75.53,45.95,15.32-7.81,77.2c-.8,7.89-4.81,15.25-11,20.2l-50.88,40.71h0Z"
                fill="#a0616a" />
            <path d="m330.69,231.49s-34-12-47,27-10,71-10,71l50,16,7-114h0Z" fill="#6c63ff" />
        ...
        <div class="text-red-400  place-self-center flex flex-col gap-0">
            <span>
                Cannot Fetch Data
            </span>
            <span class="text-xs font-semibold text-gray-500 text-center">
                Contact Admin A.S.A.P
            </span>
        </div>
    </div>
</template>
...
<script>
import { cdnIconWeather } from '../Utils/Config/cWeather'
export default {
    ...
    computed: {
        visibilityInKM() {
            // convert to km first
            return this.visibility ? this.visibility / 1000 : 0
        },

        localDate() {
            if (!this.time) false
            // Convert timestamp to milliseconds (*1000)
            return this.$timeStampToLocale(this.time * 1000, "UTC")
        }
    },
    ...
    methods: {
        async fetchWeather() {
            try {
                ...
                const { data } = await this.$axiosBase.get('')
